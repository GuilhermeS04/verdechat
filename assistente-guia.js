...
(function() {
    ...
    console.log('🤖 Assistente Guia carregado!');
    ...
    function enviarNotificacaoNativa(titulo, corpo) {
        ...
        if (Notification.permission === 'granted') {
            new Notification(titulo, { body: corpo, silent: config.intensidade === 'silencioso' });
        }
    }
    ...
    function enviarMensagemGuia(conteudo) {
        ...
        if (window.devolucoes && typeof window.devolucoes.modo === 'function' && window.devolucoes.modo()) {
            if (typeof window.addMessageDirect === 'function') {
                window.addMessageDirect('bot', `🤖 **Assistente Guia:**\n\n${conteudo}`, false);
            }
        }
    }
    ...
    function verificarAlertasPeriodicos() {
        ...
        const todas = window.devolucoes.todos();
        const pendentes = todas.filter(d => d.status === 'pendente' || d.status === 'divergente');
        ...
        if (countAntiga > 0 && lastAntiga !== hojeStr) {
            alertasParaEnviar.push(`🔴 **Alerta de Notas Antigas:** Temos **${countAntiga}** devoluções pendentes há mais de ${config.diasAntiga} dias!`);
            salvamentos.push(() => localStorage.setItem('verdechat_guia_last_antiga_alert', hojeStr));
        }
        ...
        let resetUrgenciaCount = false;
        if (hojeStr !== lastUrgenciaDate) {
            resetUrgenciaCount = true;
        }

        const totalUrgentes = countUrgencia + countAntiga;
        const currentCount = resetUrgenciaCount ? 0 : urgenciaCount;
        
        if (totalUrgentes > 0 && currentCount < 2 && (nowTime - lastUrgenciaTime) > 4 * 60 * 60 * 1000) {
            alertasParaEnviar.push(`⚠️ **Alerta de Urgência:** Existem **${totalUrgentes}** notas pendentes há mais de ${config.diasUrgente} dias.`);
            salvamentos.push(() => {
                localStorage.setItem('verdechat_guia_last_urgencia_alert_date', hojeStr);
                localStorage.setItem('verdechat_guia_urgencia_alert_count', String(currentCount + 1));
        ...
        if (totalPendentes2Dias > 0 && lastPendencia !== hojeStr) {
            alertasParaEnviar.push(`⏳ **Alerta de Pendências:** Temos **${totalPendentes2Dias}** notas paradas há 2+ dias.`);
            salvamentos.push(() => localStorage.setItem('verdechat_guia_last_pendencia_alert', hojeStr));
        }

        // Enviar alertas se houver algum acumulado
        if (alertasParaEnviar.length > 0) {
            salvamentos.forEach(f => f());

            const mensagemConsolidada = `📢 **VERDECHAT - ALERTAS CONSOLIDADOS**\n\n` + alertasParaEnviar.join('\n\n');
            enviarMensagemGuia(mensagemConsolidada);

            const resumo = `Temos ${alertasParaEnviar.length} alerta(s) de devoluções pendentes no sistema.`;
            enviarNotificacaoNativa('VerdeChat - Alertas', resumo);
        }
    }
    ...
    function mostrarConfiguracaoGuia() {
        ...
        <div style="padding: 10px; background: var(--input-bg); border-radius: 12px; border: 1px solid var(--border-color); font-size: 0.8rem; line-height: 1.4; color: var(--text-primary); max-width: 320px;">
            <div style="font-weight: bold; color: var(--accent); margin-bottom: 6px;">⚙️ CONFIGURAR GUIA</div>
