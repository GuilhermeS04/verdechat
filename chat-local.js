...
(function() {
    ...
    let lastHeartbeat = {}; // amigoId -> timestamp
    ...
    function iniciarHeartbeat() {
        ...
        heartbeatInterval = setInterval(() => {
            ...
            state.amigos.forEach(a => {
                const conn = state.conexoes[a.id];
                const lastTime = lastHeartbeat[a.id] || 0;
                const isOnline = !!(conn && conn.open && (now - lastTime < 60000));
                
                if (a.online !== isOnline) {
                    a.online = isOnline;
                    mudou = true;
                }
    ...
    function tentaConectarSilencioso(idAmigo) {
        if (!state.peer || state.conexoes[idAmigo] || idAmigo === state.peer.id) return;
        try {
            const conn = state.peer.connect(idAmigo, { reliable: true });
            conn.on('open', () => {
                console.log('✅ Conexão silenciosa estabelecida com:', idAmigo);
                state.conexoes[idAmigo] = conn;
                lastHeartbeat[idAmigo] = Date.now();
    ...
    function atualizarStatusTodos() {
        ...
        state.amigos.forEach(a => {
            ...
            if (!isOnline && state.peer && !state.peer.destroyed) {
                tentaConectarSilencioso(a.id);
            }
        });
    ...
    window.chatLocalRemoverAmigo = function(id) {
        if (confirm(`Deseja remover ${id} dos contatos?`)) {
            state.amigos = state.amigos.filter(a => a.id !== id);
            delete state.conexoes[id];
            delete state.conversas[id];
            salvarDados();
            atualizarInterface();
        }
    };
    ...
    function criarPeer() {
        ...
        let id = localStorage.getItem('verdechat_meu_id');
        if (!id) {
            id = nome.toLowerCase().replace(/\s/g, '') + '-' + Math.random().toString(36).substring(2, 6);
            localStorage.setItem('verdechat_meu_id', id);
        }
    ...
    function configurarConexao(conn, idAmigo) {
        conn.on('data', (dados) => {
            ...
            if (dados.tipo === 'ping') {
                try { conn.send({ tipo: 'pong' }); } catch(e) {}
                return;
            }
            if (dados.tipo === 'pong') {
                return;
            }
            if (dados.tipo === 'desconexao') {
                console.log('🔌 Sinal de desconexão recebido de:', idAmigo);
                const amigo = state.amigos.find(a => a.id === idAmigo);
                if (amigo) {
                    amigo.online = false;
                    salvarDados();
    ...
    function conectarAmigo(idAmigo) {
        ...
        // Garante que o amigo é adicionado à lista (offline inicialmente) sem criar duplicatas
        let amigo = state.amigos.find(a => a.id === idAmigo);
        if (!amigo) {
            const nomeAmigo = idAmigo.split('-')[0] || 'Amigo';
            amigo = { id: idAmigo, nome: nomeAmigo, online: false };
            state.amigos.push(amigo);
            salvarDados();
            atualizarInterface();
        }
        ...
        try {
            ...
            conn.on('error', (err) => {
                ...
                alert('Não foi possível conectar. O amigo continuará na sua lista como offline.');
            });
    ...
    function enviarArquivoDoInput(file) {
        ...
        reader.onload = function(e) {
            ...
            // Detecção de JSON de devoluções ao enviar
            if (file.name.toLowerCase().endsWith('.json')) {
                verificarEPerguntarImportacao(file.name, dataUrl, idAmigo);
            }
        };
        reader.readAsDataURL(file);
    }

    function verificarEPerguntarImportacao(fileName, fileData, idAmigo) {
        if (fileName.toLowerCase().endsWith('.json') && fileData) {
            try {
                const base64Part = fileData.split(',')[1];
                const jsonStr = decodeURIComponent(escape(atob(base64Part)));
                const parsed = JSON.parse(jsonStr);
                
                // Valida se é um JSON de devoluções
                if (parsed && (parsed.versao === '1.0' || parsed.versao === 1.0) && Array.isArray(parsed.devolucoes)) {
                    const dataId = 'import-' + Date.now();
                    window.__tempImportData = window.__tempImportData || {};
                    window.__tempImportData[dataId] = { dados: parsed, fileName: fileName };

                    const msgConfirmacao = {
                        tipo: 'sistema_importar',
                        nome: 'Sistema',
                        conteudo: `📎 Arquivo recebido: **"${fileName}"** | 🔍 Detectei que é um arquivo de devoluções! Deseja importar?<br><br>
                        <div style="display:flex; gap:10px; margin-top:8px;">
                            <button onclick="window.importarDevolucaoDirect('${fileData}', '${fileName}'); this.closest('.bubble').innerHTML='✅ Importação iniciada!';" style="background:var(--accent); color:white; border:none; padding:4px 10px; border-radius:8px; cursor:pointer; font-size:0.75rem; font-weight:bold;">✅ Sim</button>
                            <button onclick="this.closest('.bubble').innerHTML='❌ Importação cancelada.';" style="background:var(--input-bg); color:var(--text-secondary); border:1px solid var(--border-color); padding:4px 10px; border-radius:8px; cursor:pointer; font-size:0.75rem;">❌ Não</button>
                        </div>`,
                    ...
                    state.conversas[idAmigo].push(msgConfirmacao);
                    salvarDados();
                    atualizarInterface();
                }
            } catch(err) {
                console.error('Erro ao verificar JSON de devoluções:', err);
            }
        }
    }
    ...
    function receberMensagem(idAmigo, dados) {
        ...
        const msgObj = {
            ...
            hora: dados.hora && dados.hora.startsWith('[') ? dados.hora : '[' + new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) + ']',
            origem: 'amigo'
        };
        ...
        // Detecção de JSON de devoluções ao receber
        if (dados.tipo === 'arquivo' && dados.fileName && dados.fileName.toLowerCase().endsWith('.json')) {
            verificarEPerguntarImportacao(dados.fileName, dados.fileData, idAmigo);
        }
    }
    ...
    function renderizarListaAmigos() {
        const amigosLista = state.amigos.map(a => {
            ...
            const statusIcon = a.online ? '🟢' : '⚫';
            const statusText = a.online ? 'Online' : 'Offline';
            
            return `
                <div class="amigo-item" data-id="${a.id}" style="padding:10px 14px;border-radius:12px;border:1px solid var(--border-color);cursor:pointer;margin-bottom:8px;transition:all 0.2s;background:var(--chat-bg);display:flex;justify-content:space-between;align-items:center;">
                    <div style="flex:1; min-width:0; margin-right:10px;">
                        <div style="display:flex; align-items:center; gap:6px; flex-wrap:wrap;">
                            <span style="font-size:0.9rem; color:${a.online ? 'var(--accent)' : 'var(--text-secondary)'};">${statusIcon}</span>
                            <span style="color:var(--text-primary);font-weight:600;font-size:0.9rem;">${a.nome}</span>
                            <span style="color:var(--text-secondary);font-size:0.75rem;">(${a.id})</span>
                            <span style="color:var(--text-secondary);font-size:0.75rem;">- ${statusText}</span>
                        </div>
                        ${ultima ? `<div style="font-size:0.72rem;color:var(--text-secondary);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${ultima}</div>` : ''}
                    </div>
                    <div style="display:flex; align-items:center; gap:8px; flex-shrink:0;">
                        ${novas > 0 ? `<span style="background:var(--accent);color:white;padding:2px 8px;border-radius:20px;font-size:0.7rem;font-weight:bold;">${novas}</span>` : ''}
                        <button onclick="window.chatLocalRemoverAmigo('${a.id}'); event.stopPropagation();" title="Remover Amigo" style="background:#c0392b; color:white; border:none; padding:4px 8px; border-radius:6px; cursor:pointer; font-size:0.72rem; font-weight:bold; font-family:inherit;">❌ Remover</button>
                    </div>
                </div>
            `;
        }).join('');
            ...
            <div style="display:flex;flex-direction:column;height:100%;">
                ...
                <div style="padding:12px 16px;border-bottom:1px solid var(--border-color);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
                    <div style="display:flex;align-items:center;gap:10px;">
                        <span style="font-size:1.1rem;color:var(--text-primary);font-weight:bold;">📋 MEUS AMIGOS</span>
                        <span style="color:var(--text-secondary);font-size:0.8rem;">👤 ${state.nome}</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:10px;">
                        <span id="statusConexao" style="color:var(--text-secondary);font-size:0.75rem;">${state.peer ? '🟢 Conectado' : '🔄 Carregando...'}</span>
                ...
                <!-- Conectar -->
                <div style="padding:8px 16px;background:var(--topbar-bg);border-bottom:1px solid var(--border-color);display:flex;gap:8px;align-items:center;flex-shrink:0;flex-wrap:wrap;">
                    <span style="color:var(--text-secondary);font-size:0.75rem;">Adicionar amigo:</span>
                ...
                <div style="padding:10px 16px; display:flex; gap:10px; flex-shrink:0; border-top:1px solid var(--border-color); background:var(--topbar-bg);">
                    <button id="limparOfflinesBtn" style="flex:1; background:#c0392b; color:white; border:none; padding:8px; border-radius:8px; cursor:pointer; font-weight:bold; font-size:0.8rem; font-family:inherit;">🧹 Limpar Offlines</button>
                    <button id="atualizarStatusBtn" style="flex:1; background:var(--accent); color:white; border:none; padding:8px; border-radius:8px; cursor:pointer; font-weight:bold; font-size:0.8rem; font-family:inherit;">🔄 Atualizar</button>
                </div>
    ...
    function renderizarConversa() {
        ...
        const msgsHtml = mensagens.map(m => {
            ...
            const formattedTime = m.hora ? (m.hora.startsWith('[') ? m.hora : `[${m.hora}]`) : '';
            return `
                <div style="margin-bottom:8px;${m.origem === 'eu' ? 'text-align:right;' : ''}">
                    <div style="display:inline-block;max-width:85%;padding:8px 14px;border-radius:12px;background:${m.origem === 'eu' ? 'var(--accent)' : 'var(--input-bg)'};color:${m.origem === 'eu' ? 'white' : 'var(--text-primary)'};">
                        <div style="font-size:0.65rem;opacity:0.7;margin-bottom:2px;text-align:left;">${formattedTime} ${m.nome}</div>
                        ${conteudoHtml}
                    </div>
                </div>
            `;
    ...
    function configurarEventos() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target.id === 'limparOfflinesBtn' || e.target.closest('#limparOfflinesBtn')) {
                state.amigos = state.amigos.filter(a => a.online);
                salvarDados();
                atualizarInterface();
            }
        });
        ...
        document.addEventListener('click', (e) => {
            if (e.target.id === 'atualizarStatusBtn' || e.target.closest('#atualizarStatusBtn')) {
                atualizarStatusTodos();
            }
        });

        window.addEventListener('beforeunload', () => {
            for (const [idAmigo, conn] of Object.entries(state.conexoes)) {
                try {
                    conn.send({ tipo: 'desconexao' });
                    conn.close();
                } catch(e) {}
            }
        });
